...
import {
  ...
  TableBody,
  TableCell,
} from '@/components/ui/table' // Ajusta la ruta a tus componentes de UI
...
const columns: ColumnDef<User>[] = [
  ...
  },
  {
    ...
    header: 'Editar',
    cell: ({ row }) => (
      <Button className="bg-transparent border-0 p-0 cursor-pointer w-16" variant="outline" size="sm">
...
export function TransactionsTable() {
  ...
  const table = useReactTable({
    ...
    columns,
    state: {
      // Establece el filtro para la columna "name"
      columnFilters: [{ id: 'name', value: nameFilter }],
      pagination,
    },
    onColumnFiltersChange: (updater) => {
      // Si updater es una función, la llamamos con un array vacío como estado inicial.
      const filters =
    ...
    manualPagination: false, // Usamos paginación en el lado del cliente
  })
